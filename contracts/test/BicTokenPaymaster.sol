...
contract BicTokenPaymaster is BasePaymaster, ERC20Votes, Pausable {
    ...
    /// The start date timestamp to calculate tax reduction
    uint256 public taxFeeIntervalTimesamp;

    /// The interval step to calculate tax reduction
    uint public taxFeeIntervalStep;

    /// The period to calculate tax reduction
    uint256 public taxFeePeriod;

    /// enable reduce taxFee interval
    bool public isEnableReduceTaxFeePeriod;
    ...
    /// @dev Emitted when tax fee interval step is upadted
    event UpdateTaxFeeIntervalStep(uint256 _taxFeeIntervalStep);

    /// @dev Emitted when tax fee period is upadted
    event UpdateTaxFeePeriod(uint256 _taxFeePeriod);

    /// @dev Emitted when tax fee interval timestamp is upadted
    event UpdateTaxFeeIntervalTimestamp(uint256 _taxFeeIntervalTimesamp);

    /// @dev Emitted when isEnableReduceTaxFeePeriod is upadted
    event SetIsEnableReduceTaxFeePeriod(bool isEnableReduceTaxFeePeriod);
    ...
    constructor(IEntryPoint _entryPoint, address _owner) ERC20("Beincom", "BIC") BasePaymaster(_entryPoint) ERC20Permit("Beincom") {
        ...
        buyTax = 0;
        sellTax = 2000;
        taxFeeIntervalStep = 100;
        taxFeePeriod = 60 * 60 * 24 * 30; // 30 days
        isEnableReduceTaxFeePeriod = true;
        taxFeeIntervalTimesamp = block.timestamp;
    ...
    /**
     * @notice Update tax fee interval step
     * @param _taxFeeIntervalStep tax fee interval step
     */
    function setTaxFeeIntervalStep(uint256 _taxFeeIntervalStep) external onlyOwner {
        require(_taxFeeIntervalStep > 0, "Tax fee interval step must be greater than 0");
        taxFeeIntervalStep = _taxFeeIntervalStep;
        emit UpdateTaxFeeIntervalStep(taxFeeIntervalStep);
    }

    /**
     * @notice Update tax fee period
     * @param _taxFeePeriod tax fee period
     */
    function setTaxFeePeriod(uint256 _taxFeePeriod) external onlyOwner {
        require(_taxFeePeriod > 0, "Tax fee period must be greater than 0");
        taxFeePeriod = _taxFeePeriod;
        emit UpdateTaxFeePeriod(taxFeePeriod);
    }

    /**
     * @notice Get current tax
     * @param _tax tax
     * @return current tax
     */
    function getCurrentTax(uint256 _tax) public view returns (uint256) {
        if(!isEnableReduceTaxFeePeriod) {
            return _tax;
        }
        uint256 timeDifference = block.timestamp - taxFeeIntervalTimesamp;
        uint256 periodPassed = timeDifference.div(taxFeePeriod);

        uint256 totalReduction = periodPassed.mul(taxFeeIntervalStep);
        if(_tax > totalReduction) {
            return _tax.sub(totalReduction);
        } else {
            return 0;
        }
    }   
    ...
    /**
     * @notice Update tax fee interval timesamp
     * @param _newTaxFeeIntervalTimesamp new tax fee interval timesamp
     */
    function setTaxFeeIntervalTimestamp(uint256 _newTaxFeeIntervalTimesamp) external onlyOwner {
        require(_newTaxFeeIntervalTimesamp > 0, "Tax fee interval timesamp must be greater than 0");
        taxFeeIntervalTimesamp = _newTaxFeeIntervalTimesamp;
        emit UpdateTaxFeeIntervalTimestamp(taxFeeIntervalTimesamp);
    }

    /**
     * @notice Update enable reduce tax fee period
     * @param _isEnableReduceTaxFeePeriod enable/unenable reduce tax fee period
     */
    function setIsEnableReduceTaxFeePeriod(bool _isEnableReduceTaxFeePeriod) external onlyOwner {
        isEnableReduceTaxFeePeriod = _isEnableReduceTaxFeePeriod;
        emit SetIsEnableReduceTaxFeePeriod(isEnableReduceTaxFeePeriod);
    }
    ...
    function setIsExcludedFromTax(address excludedAddress, bool status) external onlyOwner {
        _setIsExcludedFromTax(excludedAddress, status);
    }
     ...
     * @param status status of excluded address
     */
    function bulkExcludedFromFee(address[] memory excludedAddresses, bool status) external onlyOwner {
        for (uint256 i = 0; i < excludedAddresses.length; i++) {
            _setIsExcludedFromTax(excludedAddresses[i], status);
        }
    }
     ...
     */
    function withdrawStuckToken(address token, address to) public onlyOwner {
        bool success;
        if (token == address(0)) {
            ...
            require(amount > 0, "0 amount token");
     ...
     */
    function _setIsExcludedFromTax(address _excludedAddress, bool _status) internal {
        _isExcludedFromTax[_excludedAddress] = _status;
        emit ExcludedFromTaxUpdated(_excludedAddress, _status);
    }
        ...
        if(sellTax > 0 || buyTax > 0)
        {
            ...
            if (_isTakenTax) {
                ...
                if (_isPool[to] && sellTax > 0) {
                    _tax = amount.mul(getCurrentTax(sellTax)).div(10000);
                }else if (_isPool[from] && buyTax > 0) {
                    _tax = amount.mul(getCurrentTax(buyTax)).div(10000);
                }
            }
